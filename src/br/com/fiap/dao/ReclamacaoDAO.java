...
public class ReclamacaoDAO {
	...
	public Reclamacao consultarPorId(int numero) throws Exception {
		...
		if (rs.next()) {
			...
			new Reclamacao(rs.getInt("ID_EVENTO"), new Usuario(rs.getInt("ID_USUARIO")),
					new Topico(rs.getInt("id_Topico")), rs.getString("dt_Abertura"), rs.getString("ds_Evento"),
					rs.getString("nm_evento"), rs.getInt("ID_RECLAMACAO"), rs.getString("ds_nome_adm"),
					rs.getString("ds_nome_usuario"), rs.getString("imagem"));
		}
		rs.close();
		return rcl;
	}
	...
	public List<Reclamacao> consultarNomeADM(String adm) throws Exception {
		...
		while (rs.next()) {
			lista.add(new Reclamacao(rs.getInt("ID_EVENTO"), new Usuario(rs.getInt("ID_USUARIO")),
					new Topico(rs.getInt("id_Topico")), rs.getString("dt_Abertura"), rs.getString("ds_Evento"),
					rs.getString("nm_evento"), rs.getInt("ID_RECLAMACAO"), rs.getString("ds_nome_adm"),
					rs.getString("ds_nome_usuario"), rs.getString("imagem")));
	...
	public int atualizararReclamacao(Reclamacao rec) throws Exception {
		stmt = con.prepareStatement(
				"UPDATE T_DDD_RECLAMACAO SET ds_nome_adm=?,ds_nome_usuario=?,imagem=? WHERE ID_RECLAMACAO = ?");
