...
public class GrupoUsuarioDAO {
	...
	public GrupoUsuario consultarPorIdUsuario(int numero) throws Exception {
		...
		if (rs.next()) {
			gpus.setIdGrupo(new Grupo(rs.getInt("ID_GRUPO"), rs.getString("DS_NOME")));
			gpus.setIdUsuario(new Usuario(rs.getInt("ID_USUARIO"), rs.getString("ds_java"), rs.getInt("ds_tipo"),
