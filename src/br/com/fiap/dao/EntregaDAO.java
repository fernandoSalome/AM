...
public class EntregaDAO {
	...
	public List<Entrega> consultarPorData(String dt) throws Exception {
		...
		rs = stmt.executeQuery();
		while (rs.next()) {
			lista.add(new Entrega(rs.getInt("ID_ENTREGA"), new Usuario(rs.getInt("ID_USUARIO")),
					rs.getString("DT_ENTREGA"), rs.getString("NM_REMETENTE"), rs.getString("DS_PACCOTE"),
					rs.getInt("QT_ENTREGAS_ESPERA")));
		}
		rs.close();
		return lista;
	...
	public int deletar(int numero) throws Exception {
		stmt = con.prepareStatement("DELETE * FROM T_DBC_ENTREGA WHERE ID_ENTREGA = ?");
		stmt.setInt(1, numero);
		return stmt.executeUpdate();
	}
