...
public class ReservaSalaoFestaDAO {
	...
	public ReservaSalaoFesta consultarId(int numero) throws Exception {
		...
		if (rs.next()) {
			...
			rsv.setIdCronograma(new Cronograma(rs.getInt("ID_cronograma"), rs.getString("dt_data")));
			rsv.setIdUsuario(new Usuario(rs.getInt("ID_USUARIO")));
		}
		rs.close();
		return rsv;
	}
	...
	public int atualizarDtReserva(int numero) throws Exception {
		stmt = con.prepareStatement("UPDATE T_DBC_ReservaSalaoFesta SET dt_reserva WHERE ID_ReservaSalaoFesta");
		stmt.setInt(1, numero);
		return stmt.executeUpdate();
	}

	public int deletar(int numero) throws Exception {
		stmt = con.prepareStatement("DELETE FROM T_dBC_ReservaSalaoFesta where id_ReservaSalaoFesta");
		stmt.setInt(1, numero);
		return stmt.executeUpdate();
	}
