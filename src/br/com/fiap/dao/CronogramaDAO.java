...
public class CronogramaDAO {
	...
	public String criarCronograma(Cronograma crn) throws Exception {
		stmt = con.prepareStatement("INSERT INTO T_DBC_CRONOGRAMA (ID_CRONOGRAMA,DT_DATA) VALUES (?,?)");
	...
	public Cronograma consultarPorId(int numero) throws Exception {
		stmt = con.prepareStatement("SELECT * FROM  t_dbc_cronograma WHERE id_cronograma = ?");
	...
	public List<Cronograma> consultarPorData(String data) throws Exception {
		...
		stmt = con.prepareStatement("SELECT * FROM T_DBC_CRONOGRAMA WHERE dt_data = ?");
		stmt.setString(1, data);
		rs = stmt.executeQuery();
		while (rs.next()) {
			lista.add(new Cronograma(rs.getInt("id_cronograma"), rs.getString("dt_data")));
		}
		rs.close();
		return lista;
	}
